...
region_servers=$(docker exec hbase-master bash -c "echo 'status' | hbase shell 2>&1 | grep -o '[0-9] servers' | head -1 | cut -d' ' -f1" 2>/dev/null || echo "0")
...
# 检查表分布
print_info "   检查表分布状态..."
table_status=$(docker exec hbase-master bash -c "echo 'status \"detailed\"' | hbase shell 2>&1 | grep -A 5 'test_table' | head -3")
...
if docker exec hbase-master bash -c "echo 'create \"test_check_table\", \"cf1\"' | hbase shell 2>&1 | grep -q 'Created table'" 2>/dev/null; then
    ((success_count++))
    echo "基本功能正常"
    # 清理临时检查表
